...
func Test_provideConfigValue(t *testing.T) {
	...
	t.Run("should provide config value as int64", func(t *testing.T) {
		cfg := viper.New()
		configKey := "int-cfg-key"
		cfg.Set(configKey, rand.Int64N(1000))

		type configReceiver struct {
			dig.In
			Value int64 `name:"config.int-cfg-key"`
		}

		container := dig.New()
		require.NoError(t, di.ProvideAll(container, provideConfigValue(cfg, configKey).asInt64()))

		require.NoError(t, container.Invoke(func(receiver configReceiver) {
			require.Equal(t, cfg.GetInt64(configKey), receiver.Value)
		}))
	})
